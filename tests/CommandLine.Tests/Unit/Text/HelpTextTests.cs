...
{
    private readonly HeadingInfo headingInfo = new("CommandLine.Tests.dll", "1.9.4.131");
    ...
    public void Create_instance_without_options(bool newlineBetweenSections)
    {
        // Fixture setup
        // Exercize system 
        var sut = new HelpText(new HeadingInfo("Unit-tests", "2.0"), new CopyrightInfo(true, "Author", 2005, 2013));
        sut.AddNewLineBetweenHelpSections = newlineBetweenSections;
        sut.AddPreOptionsLine("pre-options line 1").AddPreOptionsLine("pre-options line 2")
            .AddPostOptionsLine("post-options line 1").AddPostOptionsLine("post-options line 2");
    ...
    public void Create_instance_with_options(bool newlineBetweenSections)
    {
        // Fixture setup
        // Exercize system 
        HelpText sut = new HelpText { AddDashesToOption = true, AddNewLineBetweenHelpSections = newlineBetweenSections }
            .AddPreOptionsLine("pre-options")
            .AddOptions(
                new NotParsed<Simple_Options>(TypeInfo.Create(typeof(Simple_Options)), Enumerable.Empty<Error>()))
            .AddPostOptionsLine("post-options");

        // Verify outcome
        var expected = new[]
        {
            "", "pre-options", "", "--stringvalue         Define a string value here.",
            ...
            "value pos. 0          Define a long value here.", "", "post-options"
        };
    ...
    public void Create_instance_with_enum_options_enabled()
    {
        ...
        // Exercize system 
        HelpText sut =
            new HelpText { AddDashesToOption = true, AddEnumValuesToHelpText = true, MaximumDisplayWidth = 80 }
                .AddPreOptionsLine("pre-options")
                .AddOptions(
                    new NotParsed<Options_With_Enum_Having_HelpText>(
                        TypeInfo.Create(typeof(Options_With_Enum_Having_HelpText)),
                        Enumerable.Empty<Error>())).AddPostOptionsLine("post-options");
    ...
    public void Create_instance_with_enum_options_disabled()
    {
        // Fixture setup
        // Exercize system 
        HelpText sut = new HelpText { AddDashesToOption = true }.AddPreOptionsLine("pre-options").AddOptions(
            new NotParsed<Options_With_Enum_Having_HelpText>(
                TypeInfo.Create(typeof(Options_With_Enum_Having_HelpText)),
                Enumerable.Empty<Error>())).AddPostOptionsLine("post-options");
    ...
    public void When_defined_MetaValue_should_be_rendered()
    {
        // Fixture setup
        // Exercize system 
        HelpText sut = new HelpText("Meta Value.").AddOptions(
            new NotParsed<Options_With_MetaValue>(
                TypeInfo.Create(typeof(Options_With_MetaValue)),
    ...
    public void When_help_text_is_longer_than_width_it_will_wrap_around_as_if_in_a_column_given_width_of_40()
    {
        ...
        lines[2].Should()
            .BeEquivalentTo(
                "  v, verbose    This is the description"); //"The first line should have the arguments and the start of the Help Text.");
    ...
    public void When_help_text_is_longer_than_width_it_will_wrap_around_as_if_in_a_column_given_width_of_100()
    {
        ...
        // Verify outcome
        var lines = sut.ToString().Split(new[] { Environment.NewLine }, StringSplitOptions.None);
        lines[2].Should().BeEquivalentTo(
            "  v, verbose    This is the description of the verbosity to test out the wrapping capabilities of"); //"The first line should have the arguments and the start of the Help Text.");
        //string formattingMessage = "Beyond the second line should be formatted as though it's in a column.";
    ...
    public void When_help_text_has_hidden_option_it_should_not_be_added_to_help_text_output()
    {
        ...
        lines[2].Should()
            .BeEquivalentTo(
                "  v, verbose    This is the description of the verbosity to test out the"); //"The first line should have the arguments and the start of the Help Text.");
        //string formattingMessage = "Beyond the second line should be formatted as though it's in a column.";
    ...
    public void Long_pre_and_post_lines_without_spaces()
    {
        ...
        sut.MaximumDisplayWidth = 40;
        sut.AddPreOptionsLine("Before 0123456789012345678901234567890123456789012 After").AddOptions(
            new NotParsed<Simple_Options_Without_HelpText>(
                TypeInfo.Create(typeof(Simple_Options_Without_HelpText)),
                Enumerable.Empty<Error>())).AddPostOptionsLine("Before 0123456789012345678901234567890123456789 After");
    ...
    {
        // Fixture setup
        var optionsInGroup = new List<NameInfo> { new("t", "testOption1"), new("c", "testOption2") };
        var fakeResult = new NotParsed<object>(
            TypeInfo.Create(typeof(NullInstance)),
            new Error[]
            {
                new BadFormatTokenError("badtoken"), new MissingValueOptionError(new NameInfo("x", "switch")),
                new UnknownOptionError("unknown"), new MissingRequiredOptionError(new NameInfo("", "missing")),
                new SequenceOutOfRangeError(new NameInfo("s", "sequence")), new NoVerbSelectedError(),
                new BadVerbSelectedError("badverb"), new HelpRequestedError(), // should be ignored
        ...
        Func<IEnumerable<MutuallyExclusiveSetError>, string> fakeMutExclRenderer = _ => string.Empty;
    ...
    public void Invoke_AutoBuild_for_Options_returns_appropriate_formatted_text()
    {
        ...
        var fakeResult = new NotParsed<Simple_Options>(
            ...
            new Error[] { new BadFormatTokenError("badtoken"), new SequenceOutOfRangeError(new NameInfo("i", "")) });
    ...
    public void Invoke_AutoBuild_for_Verbs_with_specific_verb_returns_appropriate_formatted_text()
    {
        ...
        var fakeResult = new NotParsed<object>(
            TypeInfo.Create(typeof(NullInstance)),
            new Error[] { new HelpVerbRequestedError("commit", typeof(Commit_Verb), true) });
        ...
        lines[2].Should().BeEmpty();
        lines[3].Should()
            .BeEquivalentTo("-p, --patch      Use the interactive patch selection interface to chose which");
    ...
    [Fact]
    public void
        Invoke_AutoBuild_for_Verbs_with_specific_verb_returns_appropriate_formatted_text_given_display_width_100()
    {
        // Fixture setup
        var fakeResult = new NotParsed<object>(
            TypeInfo.Create(typeof(NullInstance)),
        ...
        // Exercize system
        var helpText = HelpText.AutoBuild(fakeResult, 100);
    ...
    public void Invoke_AutoBuild_for_Verbs_with_unknown_verb_returns_appropriate_formatted_text()
    {
        ...
        var fakeResult = new NotParsed<object>(
            TypeInfo.Create(typeof(NullInstance), verbTypes),
    ...
    public void Create_instance_with_options_and_values()
    {
        // Fixture setup
        // Exercize system 
        HelpText sut = new HelpText { AddDashesToOption = true, AdditionalNewLineAfterOption = false }
            .AddPreOptionsLine("pre-options")
            .AddOptions(
                new NotParsed<Options_With_HelpText_And_MetaValue>(
    ...
    {
        // Fixture setup
        ParserResult<Options_With_Usage_Attribute> result = new NotParsed<Options_With_Usage_Attribute>(
    ...
    {
        // Fixture setup
        CultureHandlers handlers = new CultureInfo("en-US").MakeCultureHandlers();
        var fakeResult = new NotParsed<Options_With_Default_Set_To_Sequence>(
    ...
    {
        var expectedCopyright = $"Copyright (C) {DateTime.Now.Year} author";
        ...
        ParserResult<Simple_Options> fakeResult = new NotParsed<Simple_Options>(
            TypeInfo.Create(typeof(Simple_Options)),
            new Error[0]);
        var actualResult = HelpText.AutoBuild(fakeResult, ht => ht, ex => ex);
    ...
    public void AutoBuild_with_assembly_title_and_version_attributes_only()
    {
        ...
        ReflectionHelper.SetAttributeOverride(
            new Attribute[]
            {
    ...
    {
        var expectedCompany = "Company";

        ReflectionHelper.SetAttributeOverride(new Attribute[] { new AssemblyCompanyAttribute(expectedCompany) });

        ParserResult<Simple_Options> fakeResult = new NotParsed<Simple_Options>(
            TypeInfo.Create(typeof(Simple_Options)),
            new Error[0]);
        var onErrorCalled = false;
        var actualResult = HelpText.AutoBuild(
            fakeResult,
            ht =>
            {
                onErrorCalled = true;
    ...
    public void Add_line_with_two_empty_spaces_at_the_end()
    {
        ...
        HelpText.AddLine(b, "Test  ", 1);

        Assert.Equal(
            "T" + Environment.NewLine + "e" + Environment.NewLine + "s" + Environment.NewLine + "t",
            b.ToString());
    }
    ...
    public void HelpTextHonoursLineBreaks()
    {
        // Fixture setup
        // Exercize system 
        HelpText sut = new HelpText { AddDashesToOption = true }.AddOptions(
            new NotParsed<Simple_Options>(
                TypeInfo.Create(typeof(HelpTextWithLineBreaks_Options)),
                Enumerable.Empty<Error>()));
    ...
    public void HelpTextPreservesIndentationAcrossWordWrap()
    {
        // Fixture setup
        // Exercise system 
        HelpText sut = new HelpText { AddDashesToOption = true, MaximumDisplayWidth = 60 }.AddOptions(
            new NotParsed<Simple_Options>(
                TypeInfo.Create(typeof(HelpTextWithLineBreaksAndSubIndentation_Options)),
                Enumerable.Empty<Error>()));
    ...
    public void HelpTextIsConsitentRegardlessOfCompileTimeLineStyle()
    {
        ...
        HelpText sut = new HelpText { AddDashesToOption = true }.AddOptions(
            new NotParsed<Simple_Options>(
                TypeInfo.Create(typeof(HelpTextWithMixedLineBreaks_Options)),
                Enumerable.Empty<Error>()));
    ...
    public void HelpTextPreservesIndentationAcrossWordWrapWithSmallMaximumDisplayWidth()
    {
        // Fixture setup
        // Exercise system 
        HelpText sut = new HelpText { AddDashesToOption = true, MaximumDisplayWidth = 10 }.AddOptions(
            new NotParsed<Simple_Options>(
                TypeInfo.Create(typeof(HelpTextWithLineBreaksAndSubIndentation_Options)),
                Enumerable.Empty<Error>()));
    ...
    {
        HelpText sut = new HelpText(headingInfo) { AddDashesToOption = true, MaximumDisplayWidth = 100 }.AddOptions(
            new NotParsed<Simple_Options_With_OptionGroup>(
                TypeInfo.Create(typeof(Simple_Options_With_OptionGroup)),
                Enumerable.Empty<Error>()));
    ...
    {
        HelpText sut = new HelpText(headingInfo) { AddDashesToOption = true, MaximumDisplayWidth = 100 }.AddOptions(
            new NotParsed<Simple_Options_With_Required_OptionGroup>(
                TypeInfo.Create(typeof(Simple_Options_With_Required_OptionGroup)),
                Enumerable.Empty<Error>()));
    ...
    {
        HelpText sut = new HelpText(headingInfo) { AddDashesToOption = true, MaximumDisplayWidth = 100 }.AddOptions(
            new NotParsed<Simple_Options_With_Multiple_OptionGroups>(
                TypeInfo.Create(typeof(Simple_Options_With_Multiple_OptionGroups)),
                Enumerable.Empty<Error>()));
    ...
    {
        var expectedCopyright = "Copyright (c) 2019 Global.com";
    ...
    public void AutoBuild_with_custom_help_and_version_request()
    {
        ...
        var helpText = HelpText.AutoBuild(fakeResult, ht => ht);
    ...
    [Trait("Category", "CustomHelp")]
    public void
        Invoke_Custom_AutoBuild_for_Verbs_with_specific_verb_and_no_AdditionalNewLineAfterOption_returns_appropriate_formatted_text()
    {
        // Fixture setup
        var fakeResult = new NotParsed<object>(
            TypeInfo.Create(typeof(NullInstance)),

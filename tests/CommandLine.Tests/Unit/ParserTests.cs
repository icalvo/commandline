...
public class ParserTests
{
    ...
    public void Parse_options_with_double_dash()
    {
        ...
        // Exercize system
        var result = sut.ParseArguments<Simple_Options_With_Values>(
    ...
    public void Parse_options_with_repeated_value_in_values_sequence_and_option()
    {
        ...
        Options_With_Value_Sequence_And_Normal_Option options =
            (result as Parsed<Options_With_Value_Sequence_And_Normal_Option>).Value;
        options.Compress.Should().BeEquivalentTo("x1");
        options.InputDirs.Should().BeEquivalentTo("x1", "x2", "x3");
    }
    ...
    public void Parse_options_with_double_dash_and_option_sequence()
    {
        ...
        // Exercize system
        var result = sut.ParseArguments<Options_With_Option_Sequence_And_Value_Sequence>(
            new[] { "--option-seq", "option1", "option2", "option3", "--", "value1", "value2", "value3" });

        // Verify outcome
        ((Parsed<Options_With_Option_Sequence_And_Value_Sequence>)result).Value.Should()
    ...
    public void Parse_verbs()
    {
        ...
        {
            Quiet = true, Urls = new[] { "http://gsscoder.github.com/", "http://yes-to-nooo.github.com/" }
        };
        var sut = new Parser();
    ...
    public void Parse_options_with_short_name_in_verbs_scenario(string message, string[] args)
    {
        ...
        // Exercize system
        var result = sut.ParseArguments(args, typeof(Add_Verb), typeof(Commit_Verb), typeof(Clone_Verb));
    ...
    public void Parse_verbs_using_generic_overload()
    {
        ...
        // Exercize system
        var result = sut.ParseArguments<Add_Verb, Commit_Verb, Clone_Verb>(
            new[] { "clone", "-q", "http://gsscoder.github.com/", "http://yes-to-nooo.github.com/" });
    ...
    {
        // Fixture setup
        var expectedOptions = new Immutable_Simple_Options("strvalue", new[] { 1, 2, 3 }, default, default);
        var sut = new Parser();
    ...
    public void Help_screen_in_default_verb_scenario()
    {
        ...
        // Exercise system
        sut.ParseArguments<Add_Verb_As_Default, Commit_Verb, Clone_Verb>(new[] { "--help" });
        var result = help.ToString();
    ...
    public void Errors_of_type_MutuallyExclusiveSetError_are_properly_formatted()
    {
        ...
        // Exercize system
        sut.ParseArguments<Options_With_Two_Option_Required_Set_To_True_And_Two_Sets>(
    ...
    public void Properly_formatted_help_screen_is_displayed_when_usage_is_defined_in_verb_scenario()
    {
        ...
        // Exercize system
        sut.ParseArguments<Add_Verb_With_Usage_Attribute, Commit_Verb_With_Usage_Attribute,
            Clone_Verb_With_Usage_Attribute>(new[] { "clone", "--badoption=@bad?value" });
        var result = help.ToString();
        ...
        lines[11].Should()
            .BeEquivalentTo("--no-hardlinks    Optimize the cloning process from a repository on a local");
    ...
    [Fact]
    public void
        Properly_formatted_help_screen_is_displayed_when_there_is_a_hidden_verb_selected_usage_displays_with_hidden_option()
    {
        // Fixture setup
        var help = new StringWriter();
        var sut = new Parser(config => config.HelpWriter = help);
        ...
        // Exercize system
        sut.ParseArguments<Secert_Verb, Add_Verb_With_Usage_Attribute>(new[] { "secert", "--help" });
        var result = help.ToString();
    ...
    public void Parse_options_when_given_hidden_verb()
    {
        ...
        // Exercize system
        var result = sut.ParseArguments<Secert_Verb, Add_Verb_With_Usage_Attribute>(new[] { "secert", "--force" });
    ...
    public void Parse_options_when_given_hidden_verb_with_hidden_option()
    {
        ...
        // Exercize system
        var result =
            sut.ParseArguments<Secert_Verb, Add_Verb_With_Usage_Attribute>(
    ...
    public void Specific_verb_help_screen_should_be_displayed_regardless_other_argument()
    {
        ...
        // Exercize system
        sut.ParseArguments<Add_Verb, Commit_Verb, Clone_Verb>(new[] { "help", "clone", "extra-arg" });
        var result = help.ToString();
    ...
    public void When_IgnoreUnknownArguments_is_set_valid_unknown_arguments_avoid_a_failure_parsing(string[] arguments,
    ...
    public void When_IgnoreUnknownArguments_is_set_valid_unknown_arguments_avoid_a_failure_parsing_for_verbs(
        string[] arguments, Commit_Verb expected)
    {
        // Fixture setup
        var sut = new Parser(config => config.IgnoreUnknownArguments = true);
    ...
    public void Properly_formatted_help_screen_excludes_help_as_unknown_option()
    {
        ...
        // Exercize system
        sut.ParseArguments<Add_Verb, Commit_Verb, Clone_Verb>(new[] { "clone", "--bad-arg", "--help" });
        var result = help.ToString();
    ...
    public static void Breaking_mutually_exclusive_set_constraint_with_both_set_name_with_gererates_Error()
    {
        ...
        // Verify outcome
        ((NotParsed<Options_With_SetName_That_Ends_With_Previous_SetName>)result).Errors.Should()
    ...
    public static IEnumerable<object[]> IgnoreUnknownArgumentsData
    {
        get
        {
            ...
            {
                new[] { "--stringvalue=strdata0", "--unknown=valid" },
                new Simple_Options { StringValue = "strdata0", IntSequence = Enumerable.Empty<int>() }
            };
            yield return new object[]
            {
                new[] { "--stringvalue=strdata0", "1234", "--unknown", "-i", "1", "2", "3" },
                new Simple_Options { StringValue = "strdata0", LongValue = 1234L, IntSequence = new[] { 1, 2, 3 } }
            };
            yield return new object[]
            {
                new[] { "--stringvalue=strdata0", "-u" },
                new Simple_Options { StringValue = "strdata0", IntSequence = Enumerable.Empty<int>() }
            };
        }
    }
    ...
    public static IEnumerable<object[]> IgnoreUnknownArgumentsForVerbsData
    {
        get
        {
            ...
            yield return new object[]
            {
                new[] { "commit", "--amend", "--unknown", "valid" }, new Commit_Verb { Amend = true }
            };
        }
    }
    ...
    public static void Null_default()
    {
        ...
        parser.ParseArguments<NullDefaultCommandLineArguments>("".Split()).WithParsed(r => { Assert.Null(r.User); });
    }

    public class NullDefaultCommandLineArguments
    {
        [Option('u', "user", Default = null)] public string User { get; set; }
    }
    ...
    public void Parse_options_with_same_option_and_value_args()
    {
        ...
        parser.ParseArguments<Options_With_Option_And_Value_Of_String_Type>(
                new[] { "arg", "-o", "arg" })
            .WithNotParsed(errors => { throw new InvalidOperationException("Must be parsed."); }).WithParsed(
                args =>
                {
    ...
    public void Parse_verb_with_same_option_and_value_args()
    {
        ...
        result.WithNotParsed(errors => { throw new InvalidOperationException("Must be parsed."); })
            .WithParsed<Verb_With_Option_And_Value_Of_String_Type>(
                args =>
                {
                    Assert.Equal("arg", args.OptValue);
                    Assert.Equal("arg", args.PosValue);
    ...
    public void Parse_default_verb_implicit()
    {
        ...
        parser.ParseArguments<Default_Verb_One>(new[] { "-t" })
            ...
            .WithParsed(args => { Assert.True(args.TestValueOne); });
    }
    ...
    public void Parse_repeated_options_in_verbs_scenario_with_multi_instance()
    {
        using (var sut = new Parser(settings => settings.AllowMultiInstance = true))
        {
            ...
            var result = sut.ParseArguments(
                new[]
                {
                    "sequence", "--long-seq", $"{longVal1}", "-s", stringVal, "--long-seq", $"{longVal2};{longVal3}"
                },
                typeof(Add_Verb),
                typeof(Commit_Verb),
            ...
            result.WithParsed<SequenceOptions>(
                verb =>
                {
    ...
    public void Parse_default_verb_with_empty_name()
    {
        ...
        parser.ParseArguments<Default_Verb_With_Empty_Name>(new[] { "-t" })
            .WithNotParsed(errors => throw new InvalidOperationException("Must be parsed."))
            .WithParsed(args => { Assert.True(args.TestValue); });
    }

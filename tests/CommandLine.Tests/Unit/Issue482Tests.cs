...
public class Issue482Tests
{
    ...
    public void AutoBuild_without_ordering()
    {
        ...
        var parseResult = parser
            .ParseArguments<Options_HelpText_Ordering_Verb1, Options_HelpText_Ordering_Verb2>(
                new[] { "verb1", "--help" }).WithNotParsed(errors => { ; }).WithParsed(args => { ; });

        var message = HelpText.AutoBuild(parseResult, error => error, ex => ex);
    ...
    public void AutoBuild_with_ordering()
    {
        ...
        var comparison = HelpText.RequiredThenAlphaComparison;
    ...
    public void AutoBuild_with_ordering_on_shortName()
    {
        ...
        Comparison<ComparableOption> orderOnShortName = (attr1, attr2) =>
        {
            if (attr1.IsOption && attr2.IsOption)
            {
                if (attr1.Required && !attr2.Required) return -1;

                if (!attr1.Required && attr2.Required) return 1;

                if (string.IsNullOrEmpty(attr1.ShortName) && !string.IsNullOrEmpty(attr2.ShortName))
                    return 1;
                if (!string.IsNullOrEmpty(attr1.ShortName) && string.IsNullOrEmpty(attr2.ShortName)) return -1;

...
public class SequenceParsingTests
{
    ...
    public static void Enumerable_with_separator_before_values_does_not_try_to_parse_too_much(bool useGetoptMode)
    {
        ...
        {
            Excluded = new[] { "a", "b" }, Included = Enumerable.Empty<string>(), InputFileName = "InputFile.txt"
        };
    ...
    public static void Options_with_similar_names_are_not_ambiguous(bool useGetoptMode)
    {
        ...
        var expected = new Options_With_Similar_Names
        {
            ConfigureProfile = "deploy", Profile = "local", Deploys = Enumerable.Empty<string>()
        };
        var sut = new Parser(parserSettings => { parserSettings.GetoptMode = useGetoptMode; });
    ...
    public static void Values_with_same_name_as_sequence_option_do_not_cause_later_values_to_split_on_separators()
    {
        ...
        Type typeInfo = typeof(Options_With_Similar_Names_And_Separator);

        var specProps = typeInfo
            .GetSpecifications(
                pi => SpecificationProperty.Create(Specification.FromProperty(pi), pi, Maybe.Nothing<object>()))
            .Select(sp => sp.Specification).OfType<OptionSpecification>();

        var tokenizerResult =
            Tokenizer.ConfigureTokenizer(StringComparer.InvariantCulture, false, false)(args, specProps);
    ...
    public static void Enumerable_with_colon_separator_before_values_does_not_try_to_parse_too_much(bool useGetoptMode)
    {
        ...
        var expected = new Options_For_Issue_454 { Channels = new[] { "chanA", "chanB" }, ArchivePath = "file.hdf5" };
        var sut = new Parser(parserSettings => { parserSettings.GetoptMode = useGetoptMode; });
    ...
    public static void Enumerable_with_enum_before_values_does_not_try_to_parse_too_much(bool useGetoptMode)
    {
        ...
        var expected = new Options_For_Issue_617 { Mode = new[] { FMode.D, FMode.C }, Files = new[] { "a.txt" } };
        var sut = new Parser(parserSettings => { parserSettings.GetoptMode = useGetoptMode; });

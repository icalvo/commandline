...
public class TokenizerTests
{
    ...
    public void Explode_scalar_with_separator_in_odd_args_input_returns_sequence()
    {
        ...
        {
            Token.Name("i"), Token.Value("10"), Token.Name("string-seq"), Token.Value("aaa"), Token.Value("bb"),
            Token.Value("cccc"), Token.Name("switch")
        };
        var specs = new[]
        {
        ...
        // Exercize system
        var result = Tokenizer.ExplodeOptionList(
            Result.Succeed(
                Enumerable.Empty<Token>().Concat(
                    new[]
                    {
                        Token.Name("i"), Token.Value("10"), Token.Name("string-seq"), Token.Value("aaa,bb,cccc"),
                        Token.Name("switch")
                    }),
    ...
    public void Explode_scalar_with_separator_in_even_args_input_returns_sequence()
    {
        ...
        var expectedTokens = new[]
        {
            Token.Name("x"), Token.Name("string-seq"), Token.Value("aaa"), Token.Value("bb"), Token.Value("cccc"),
            Token.Name("switch")
        };
        var specs = new[]
        {
        ...
        var result = Tokenizer.ExplodeOptionList(
            Result.Succeed(
                Enumerable.Empty<Token>().Concat(
                    new[]
                    {
                        Token.Name("x"), Token.Name("string-seq"), Token.Value("aaa,bb,cccc"), Token.Name("switch")
                    }),
                Enumerable.Empty<Error>()),
            optionName => NameLookup.HavingSeparator(optionName, specs, StringComparer.Ordinal));
    ...
    public void Normalize_should_remove_all_value_with_explicit_assignment_of_existing_name()
    {
        ...
        var expectedTokens = new[]
        {
            Token.Name("x"), Token.Name("string-seq"), Token.Value("aaa"), Token.Value("bb"), Token.Name("unknown"),
            Token.Name("switch")
        };
        Func<string, bool> nameLookup = name => name.Equals("x") || name.Equals("string-seq") || name.Equals("switch");

        // Exercize system
        var result = Tokenizer.Normalize(
            //Result.Succeed(
            Enumerable.Empty<Token>().Concat(
                new[]
                {

...
{
    private static ParserResult<object> InvokeChoose(IEnumerable<Type> types, IEnumerable<string> arguments,
    ...
    {
        // Fixture setup
        var expected = new Immutable_Add_Verb(true, default, "dummy.bin");
    ...
    [InlineData(
        new[] { "sequence", "-s", "eml1@xyz.com,test@unit.org,xyz@srv.it" },
        new[] { "eml1@xyz.com", "test@unit.org", "xyz@srv.it" })]
    [InlineData(
        new[] { "sequence", "-sInlineData@iscool.org,test@unit.org,xyz@srv.it,another,the-last-one" },
        new[] { "InlineData@iscool.org", "test@unit.org", "xyz@srv.it", "another", "the-last-one" })]

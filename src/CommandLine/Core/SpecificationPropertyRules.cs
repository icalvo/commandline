...
internal static class SpecificationPropertyRules
{
    ...
    private static Func<IEnumerable<SpecificationProperty>, IEnumerable<Error>>
        EnforceMutuallyExclusiveSetAndGroupAreNotUsedTogether()
    {
        return specProps =>
        {
            var options = from sp in specProps
            ...
            if (options.Any())
                return from o in options select new GroupOptionAmbiguityError(new NameInfo(o.ShortName, o.LongNames));
    ...
    private static Func<IEnumerable<SpecificationProperty>, IEnumerable<Error>> EnforceGroup()
    {
        return specProps =>
        {
            var optionsValues = from sp in specProps
                where sp.Specification.IsOption()
                let o = (OptionSpecification)sp.Specification
                where o.Group.Length > 0
                select new { Option = o, sp.Value, sp.Specification.DefaultValue };

            var groups = from o in optionsValues group o by o.Option.Group into g select g;

            var errorGroups = groups.Where(gr => gr.All(g => g.Value.IsNothing() && g.DefaultValue.IsNothing()));

            if (errorGroups.Any())
                return errorGroups.Select(
                    gr => new MissingGroupOptionError(
                        gr.Key,
                        gr.Select(g => new NameInfo(g.Option.ShortName, g.Option.LongNames))));
    ...
    private static Func<IEnumerable<SpecificationProperty>, IEnumerable<Error>> EnforceMutuallyExclusiveSet()
    {
        return specProps =>
        {
            var options = from sp in specProps
                ...
                where o.SetName.Length > 0
                select o;
            var groups = from o in options group o by o.SetName into g select g;
            if (groups.Count() > 1)
                return from o in options select new MutuallyExclusiveSetError(o.FromOptionSpecification(), o.SetName);
            return Enumerable.Empty<Error>();
        };
    }
    ...
    private static Func<IEnumerable<SpecificationProperty>, IEnumerable<Error>> EnforceRequired()
    {
        return specProps =>
        {
            ...
            var requiredWithoutValue = from sp in specProps
                ...
                select sp.Specification;
            var missing = requiredWithoutValue.Except(requiredWithValue).Concat(
                ...
                where o.SetName.Length == 0
                where o.Group.Length == 0
                select sp.Specification).Concat(
                from sp in specProps
                where sp.Specification.IsValue()
                where sp.Specification.Required
                where sp.Value.IsNothing()
                select sp.Specification);
            return from sp in missing select new MissingRequiredOptionError(sp.FromSpecification());
        };
    }
    ...
    private static Func<IEnumerable<SpecificationProperty>, IEnumerable<Error>> EnforceRange()
    {
        return specProps =>
        {
            var options = specProps.Where(sp => sp.Specification.TargetType == TargetType.Sequence)
                .Where(sp => sp.Value.IsJust()).Where(
                    sp => (sp.Specification.Min.IsJust() && ((Array)sp.Value.FromJustOrFail()).Length <
                        sp.Specification.Min.FromJustOrFail()) || (sp.Specification.Max.IsJust() &&
                                                                   ((Array)sp.Value.FromJustOrFail()).Length >
                                                                   sp.Specification.Max.FromJustOrFail()));
            if (options.Any())
                return from s in options select new SequenceOutOfRangeError(s.Specification.FromSpecification());
            return Enumerable.Empty<Error>();
        };
    }
        ...
        bool allowMultiInstance)
    {
        return specProps =>
        {
            if (allowMultiInstance) return Enumerable.Empty<Error>();
    ...
    {
        if (sequence.Any()) return sequence.Contains(value);
        return true;
    }
}

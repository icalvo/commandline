...
{
    public static bool IsOption(this Specification specification) => specification.Tag == SpecificationType.Option;

    public static bool IsValue(this Specification specification) => specification.Tag == SpecificationType.Value;
    ...
    public static string UniqueName(this OptionSpecification specification) =>
        specification.ShortName.Length > 0 ? specification.ShortName : specification.LongNames[0];
    ...
    public static bool HavingRange(this Specification specification, Func<int, int, bool> predicate)
    {
        int min;
        int max;
        if (specification.Min.MatchJust(out min) && specification.Max.MatchJust(out max)) return predicate(min, max);
        return false;
    }
    ...
    {
        int min;
        if (specification.Min.MatchJust(out min)) return predicate(min);
        return false;
    }
    ...
    {
        int max;
        if (specification.Max.MatchJust(out max)) return predicate(max);
        return false;
    }
}

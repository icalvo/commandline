...
internal static class OptionMapper
{
    public static Result<IEnumerable<SpecificationProperty>, Error> MapValues(
        ...
        Func<IEnumerable<string>, Type, bool, bool, Maybe<object>> converter, StringComparer comparer)
    {
        var sequencesAndErrors = propertyTuples.Select(
            pt =>
            {
                var matched = options.Where(
                    s => s.Key.MatchName(
                ...
                {
                    var matches =
                        matched.GetValueOrDefault(Enumerable.Empty<KeyValuePair<string, IEnumerable<string>>>());
                    var values = new List<string>();
                    foreach (var kvp in matches)
                    foreach (var value in kvp.Value)
                        values.Add(value);

                    var isFlag = pt.Specification.Tag == SpecificationType.Option &&

...
public sealed class TypeInfo
{
    ...
    internal static TypeInfo Create(Type? current) => new(current, Enumerable.Empty<Type>());

    internal static TypeInfo Create(Type? current, IEnumerable<Type> choices) => new(current, choices);
}
...
/// <summary>
///     Models a parser result. When inherited by <see cref="CommandLine.Parsed{T}" />, it contains an instance of type
///     <typeparamref name="T" />
///     with parsed values.
///     When inherited by <see cref="CommandLine.NotParsed{T}" />, it contains a sequence of
///     <see cref="CommandLine.Error" />.
/// </summary>
...
{
    internal Parsed(T value, TypeInfo typeInfo) : base(value, typeInfo)
    {
    }
    ...
    {
        if (obj is Parsed<T> other) return Equals(other);
    ...
    /// <returns>
    ///     <value>true</value>
    ///     if this instance of <see cref="CommandLine.Parsed{T}" /> and <paramref name="other" /> have the same value;
    ///     otherwise,
    ///     <value>false</value>
    ///     .
    /// </returns>
    public bool Equals(Parsed<T>? other)
    {
        ...
        return Tag.Equals(other.Tag) && Equals(Value, other.Value);
    }
}
...
{
    internal NotParsed(TypeInfo typeInfo, IEnumerable<Error> errors) : base(errors, typeInfo)
    {
    }
    ...
    public override bool Equals(object? obj)
    {
        if (obj is NotParsed<T> other) return Equals(other);
    ...
    /// <returns>
    ///     <value>true</value>
    ///     if this instance of <see cref="CommandLine.NotParsed{T}" /> and <paramref name="other" /> have the same value;
    ///     otherwise,
    ///     <value>false</value>
    ///     .
    /// </returns>

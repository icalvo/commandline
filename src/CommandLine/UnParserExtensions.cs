...
public class UnParserSettings
{
    ...
    /// <summary>
    ///     Factory method that creates an instance of <see cref="CommandLine.UnParserSettings" /> with GroupSwitches set to
    ///     true.
    /// </summary>
    /// <returns>A properly initalized <see cref="CommandLine.UnParserSettings" /> instance.</returns>
    public static UnParserSettings WithGroupSwitchesOnly() => new() { GroupSwitches = true };

    /// <summary>
    ///     Factory method that creates an instance of <see cref="CommandLine.UnParserSettings" /> with UseEqualToken set to
    ///     true.
    /// </summary>
    /// <returns>A properly initalized <see cref="CommandLine.UnParserSettings" /> instance.</returns>
    public static UnParserSettings WithUseEqualTokenOnly() => new() { UseEqualToken = true };
...
public static class UnParserExtensions
{
    ...
    ///     The <see cref="Action{UnParserSettings}" /> lambda used to configure
    ...
    public static string FormatCommandLine<T>(this Parser parser, T options, Action<UnParserSettings> configuration)
    {
        ...
        var optSpecs = settings.GroupSwitches ? allOptSpecs.Where(info => !shortSwitches.Contains(info)) : allOptSpecs;
        ...
        builder = settings.GroupSwitches && shortSwitches.Any()
            ...
            : builder;
        optSpecs.ForEach(
            opt => builder.Append(FormatOption((OptionSpecification)opt.Specification, opt.Value, settings))
                .Append(' '));
    ...
    public static string[] FormatCommandLineArgs<T>(this Parser parser, T options,
        ...
        FormatCommandLine(parser, options, configuration).SplitArgs();
    ...
    private static string FormatValue(Specification spec, object value)
    {
        ...
        switch (spec.TargetType)
        {
            ...
            case TargetType.Sequence:
                ...
                Func<object, object> format = v => sep == ' ' ? FormatWithQuotesIfString(v) : v;
    ...
    private static string FormatOption(OptionSpecification spec, object value, UnParserSettings settings) =>
        new StringBuilder().Append(spec.FormatName(value, settings)).AppendWhen(
            spec.TargetType != TargetType.Switch,
            FormatValue(spec, value)).ToString();
    ...
    private static string FormatName(this OptionSpecification optionSpec, object value, UnParserSettings settings)
    {
        ...
        var formattedName = new StringBuilder(
                longName
                    ...
                    : "-".JoinTo(optionSpec.ShortName)).AppendWhen(
            optionSpec.TargetType != TargetType.Switch,
            longName && settings.UseEqualToken ? "=" : " ").ToString();
    ...
    public static string[] SplitArgs(this string command, bool keepQuote = false)
    {
        ...
        var inQuote = false;
        var chars = command.ToCharArray().Select(
            v =>
            {
                if (v == '"')
                    inQuote = !inQuote;

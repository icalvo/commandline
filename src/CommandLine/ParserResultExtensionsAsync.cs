...
public static partial class ParserResultExtensions
{
    ...
    public static async Task<ParserResult<T>> WithParsedAsync<T>(this ParserResult<T> result, Func<T, Task> action)
        where T : notnull
    {
        if (result is Parsed<T> parsed) await action(parsed.Value);
        return result;
    }
    ...
    /// <param name="action">The <see cref="Func{T, Task}" /> to execute.</param>
    /// <returns>The same <paramref name="result" /> instance as a <see cref="Task" /> instance.</returns>
    public static async Task<ParserResult<object>> WithParsedAsync<T>(this ParserResult<object> result,
        Func<T, Task> action)
    {
    ...
    /// <returns>The same <paramref name="result" /> instance as a <see cref="Task" /> instance.</returns>
    public static async Task<ParserResult<T>> WithNotParsedAsync<T>(this ParserResult<T> result,
        Func<IEnumerable<Error>, Task> action)
    {
        if (result is NotParsed<T> notParsed) await action(notParsed.Errors);
        return result;
    }
#endif
}

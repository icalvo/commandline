...
public class ParserSettings : IDisposable
{
    ...
    private int GetWindowWidth()
    {
        ...
        try
        {
            ...
            if (width < 1) width = DefaultMaximumLength;
        }
    ...
    ///     If GetoptMode is true, this defaults to true, but can be turned off by explicitly specifying EnableDashDash =
    ///     false.
    /// </summary>
    public bool EnableDashDash
    {
        get => enableDashDash.MatchJust(out var value) ? value : getoptMode;
    ...
    ///     If GetoptMode is true, this defaults to true, but can be turned off by explicitly specifying AllowMultiInstance =
    ///     false.
    /// </summary>
    public bool AllowMultiInstance
    {
        get => allowMultiInstance.MatchJust(out var value) ? value : getoptMode;
        set => PopsicleSetter.Set(Consumed, ref allowMultiInstance, Maybe.Just(value));
    }

    /// <summary>
    ///     Whether strict getopt-like processing is applied to option values; if true, AllowMultiInstance and EnableDashDash
    ///     will default to true as well.
    /// </summary>
    ...
    /// <summary>
    ///     Whether getopt-like processing should follow the POSIX rules (the equivalent of using the "+" prefix in the C
    ///     getopt() call).
    ///     If not explicitly set, will default to false unless the POSIXLY_CORRECT environment variable is set, in which case
    ///     it will default to true.
    /// </summary>
    public bool PosixlyCorrect
    {
        get =>
            posixlyCorrect.MapValueOrDefault(
                val => val,
                () => Environment.GetEnvironmentVariable("POSIXLY_CORRECT").ToBooleanLoose());

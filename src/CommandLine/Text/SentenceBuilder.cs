...
public abstract class SentenceBuilder
{
    ...
    public static SentenceBuilder Create() => Factory();
    ...
    ///     The delegates must accept a boolean that is equal
    ///     <value>true</value>
    ///     for options; otherwise
    ///     <value>false</value>
    ///     for verbs.
    /// </summary>
                ...
                return error =>
                {
                    switch (error.Tag)
                    {
                        ...
                        case ErrorType.GroupOptionAmbiguityError:
                            ...
                            return "Both SetName and Group are not allowed in option: (".JoinTo(
                ...
                return errors =>
                {
                    ...
                    var msgs = bySet.Select(
                        set =>
                        {
                            ...
                            return new StringBuilder("Option").AppendWhen(namesCount > 1, "s").Append(": ")
                                .Append(names.Substring(0, names.Length - 2)).Append(' ')
                                .AppendIf(namesCount > 1, "are", "is").Append(" not compatible with: ")
                                .Append(incompat.Substring(0, incompat.Length - 2)).Append('.').ToString();
                        }).ToArray();

...
public class CopyrightInfo
{
    ...
    public CopyrightInfo(string author, int year) : this(true, author, year)
    {
    }
    ...
    {
        if (attribute != null) return attribute.Copyright;

        return new StringBuilder(builderSize).Append(CopyrightWord).Append(' ')
            .Append(isSymbolUpper ? SymbolUpper : SymbolLower).Append(' ').Append(FormatYears(copyrightYears))
            .Append(' ').Append(author).ToString();
    }
    ...
    {
        if (years.Length == 1) return years[0].ToString(CultureInfo.InvariantCulture);
        ...
        for (var i = 0; i < years.Length; i++)
        {
            ...
            var next = i + 1;
            if (next < years.Length) yearsPart.Append(years[next] - years[i] > 1 ? " - " : ", ");
        }

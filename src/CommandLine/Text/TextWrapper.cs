...
public class TextWrapper
{
    ...
    public TextWrapper(string input)
    {
        ...
        lines = input.Replace("\r", "").Split(new[] { '\n' }, StringSplitOptions.None);
    }
    ...
    public TextWrapper WordWrap(int columnWidth)
    {
        ...
        lines = lines.SelectMany(line => WordWrapLine(line, columnWidth)).ToArray();
        return this;
    }
    ...
    {
        lines = lines.Select(line => numberOfSpaces.Spaces() + line).ToArray();
        return this;
    }
    ...
    public static string WrapAndIndentText(string input, int indentLevel, int columnWidth) =>
        new TextWrapper(input).WordWrap(columnWidth).Indent(indentLevel).ToText();
    ...
    private string[] WordWrapLine(string line, int columnWidth)
    {
        ...
        return unindentedLine.Split(' ')
            ...
            .Select(builder => currentIndentLevel.Spaces() + builder.ToString().TrimEnd()).ToArray();
    }
    ...
    private static List<StringBuilder> AddWordToLastLineOrCreateNewLineIfNecessary(List<StringBuilder> lines,
        string word, int columnWidth)
    {
    ...
    private static string RightString(string str, int n) =>
        n >= str.Length || str.Length == 0 ? string.Empty : str.Substring(n);

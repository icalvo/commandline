...
public class HeadingInfo
{
    ...
    /// </summary>
    public static HeadingInfo Empty => new("");
    ...
    public static HeadingInfo Default
    {
        get
        {
            var title = ReflectionHelper.GetAttribute<AssemblyTitleAttribute>().MapValueOrDefault(
                titleAttribute => titleAttribute.Title,
                ReflectionHelper.GetAssemblyName()) ?? "";

            var version = ReflectionHelper.GetAttribute<AssemblyInformationalVersionAttribute>().MapValueOrDefault(
    ...
    public override string ToString()
    {
        ...
        return new StringBuilder(programName.Length + version.Length + 1).Append($"{programName} {version}").ToString();
    }
    ...
    public void WriteMessage(string message, TextWriter writer)
    {
        ...
        writer.WriteLine(
            new StringBuilder(programName.Length + message.Length + 2).Append(programName).Append(": ").Append(message)
                .ToString());
    }

...
public class Parser : IDisposable
{
    ...
    /// </param>
    /// <returns>
    ///     A <see cref="CommandLine.ParserResult{T}" /> containing an instance of type <typeparamref name="T" /> with parsed
    ///     values
    ...
    ///     the array of types supplied by <paramref name="types" />.
    ///     Grammar rules are defined decorating public properties with appropriate attributes.
    ...
    private static Result<IEnumerable<Token>, Error> Tokenize(
        IEnumerable<string> arguments, IEnumerable<OptionSpecification> optionSpecs, ParserSettings settings) =>
        settings.GetoptMode
    ...
    private static ParserResult<T> MakeParserResult<T>(ParserResult<T> parserResult, ParserSettings settings) =>
        DisplayHelp(parserResult, settings.HelpWriter, settings.MaximumDisplayWidth);
